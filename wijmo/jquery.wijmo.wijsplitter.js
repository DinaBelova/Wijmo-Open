	...
	$.widget("wijmo.wijsplitter", {
		...
		_expanderMouseUp: function (e) {
			...
			if (self.options.panel1.collapsed) {
				self._trigger("expand", e, null);
				if (e.isImmediatePropagationStopped()) {
					return;
				}
			}
			else {
				self._trigger("collapse", e, null);
				if (e.isImmediatePropagationStopped()) {
					return;
				}
			}
			...
			if (self.options.panel1.collapsed) {
				self._trigger("collapsed", e, null);
			}
			else {
				self._trigger("expanded", e, null);
			}
		},

		_initResizer: function () {
			var self = this, eleW, eleH, barW, maxW, minW, barH, maxH, minH, dur;
			dur = self.options.resizeSettings.animationOptions.duration;
			if (self.options.resizeSettings.animationOptions.disabled === true) {
				dur = 0;
			}
			eleW = this.element.width();
			eleH = this.element.height();
			...
			if (this.options.orientation === "vertical") {
				...
				self._getPanel1().resizable({
					...
					animateDuration: dur,
					animateEasing: self.options.resizeSettings.animationOptions.easing,
